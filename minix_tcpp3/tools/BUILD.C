 ...
 */
  ...
  printf("fsck starts at %4.4X:%4.4X\n",cs,ip);
}
...
IOinit()			/* check if no DMAoverrun & assign the buffer */
{
  if (/*
  	DMAoverrun(buff1)
  	in the 1.1 distribution, this was a dummy routine in
  	diskio.asm; always returned 0.
  	*/
  	0)
     buff = buff2;
  else
...
char user[SECTOR_SIZE];
{
  ...
  if (drive == -1) {
	lseek(image, (long)SECTOR_SIZE * (long) blocknr, 0);
	if (read(image, user, SECTOR_SIZE) != SECTOR_SIZE)
...
char user[SECTOR_SIZE];
{
  ...
  if (drive == -1) {
	lseek(image, (long)SECTOR_SIZE * (long) blocknr, 0);
	if (write(image, user, SECTOR_SIZE) != SECTOR_SIZE)
