...
#include <stdio.h>
#include <errno.h>  /* make sure errno.h is available */
#include <ctype.h>  /* make sure ctype.h is available */
...
char **argv;
{
  ...
  if(argc >= 2) {
	...
	/*
	 * If no files specified, simply exit.  Otherwise,
	 * if not the old '-s' option, process all files.
	 * If '-s', then process files one-at-a-time.
	 */
	if(!sflag)
...
char **fnamptr;
{
	...
	for(;filecnt;)
	{
		...
		for(index = 0;(fileptr[index] != ENDLIST) && filecnt; index++)
		{
			 ...
			 * list.  In any case, output the delimiter of choice.
			 */
...
char *strptr;
{
	 ...
	 * characters has been expanded beyond that recognized by the
	 * vanilla Unix(Tm) version.
	 */
