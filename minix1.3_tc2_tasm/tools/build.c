...
unsigned long cum_size;                  /* Size of kernel+mm+fs+init */
unsigned long all_size;                  /* Size of all 5 programs */
...
/* Copy the boot block and the 5 programs to the output. */
  ...
  flush();
  printf("                                               -----      -----\n");
  printf("Operating system size  %29lX      %5lu\n", cum_size, cum_size);
  printf("\nTotal size including fsck is %lX.\n", all_size);
 ...
 */
  ...
  if ( (fd = open(file_name, O_RDONLY | O_BINARY)) < 0) pexit("can't open ",file_name);
 ...
 */
  ...
  if (num < FSCK) { 
        printf("%s  text=%5X  data=%5X  bss=%5X  tot=%5lX  dec=%lu  %s\n",
  ...
  while (left_to_read > 0) {
        ...
        if (bytes_read < 0) pexit("read error on file ", file_name);
        if (bytes_read != count) {
           printf("bytes_r not same as count pro=%d by = %x ct == %x\n",
             num,bytes_read,count);
        }
        if (bytes_read > 0) wr_out(inbuf, count);
        left_to_read -= count;
  }
...
char user[SECTOR_SIZE];
{
  ...
  retries = MAX_RETRIES;
  do
      err = absread (drive, 1, blocknr, buff);
  while (err && --retries);
...
char user[SECTOR_SIZE];
{
  ...
  retries = MAX_RETRIES;
  do
      err = abswrite (drive, 1, blocknr, buff);
  while (err && --retries);
